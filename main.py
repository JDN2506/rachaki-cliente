...
def main(page: ft.Page):
    ...
    # Função para o usuário trocar o tema (Corrigida para compatibilidade)
    def mudar_tema(e):
    ...
    def mostrar_portal():
        ...
        # Botão suspenso no canto superior direito (Corrigido)
        botao_tema = ft.Row([
            ft.PopupMenuButton(
                content=ft.Text("🎨 Tema", color="grey", weight="bold"),
                items=[
                    ft.PopupMenuItem(content=ft.Text("Sistema"), data="Sistema", on_click=mudar_tema),
                    ft.PopupMenuItem(content=ft.Text("Claro"), data="Claro", on_click=mudar_tema),
                    ft.PopupMenuItem(content=ft.Text("Escuro"), data="Escuro", on_click=mudar_tema),
                ]
            )
        ], alignment=ft.MainAxisAlignment.END)

        # Coluna centralizada com os botões esticando apenas dentro do limite dela
        coluna_central = ft.Column([
            ft.Text("Bem-vindo ao Rachaki", size=35, weight="bold", color="blue", text_align=ft.TextAlign.CENTER),
            ft.Text("Selecione o seu perfil de acesso abaixo:", size=18, color="grey", text_align=ft.TextAlign.CENTER),
            ft.Divider(height=30, color="transparent"),
            ft.Button("📱 Sou Cliente (Acessar Mesa)", on_click=lambda e: mostrar_simulador_qr(), height=60, style=ft.ButtonStyle(bgcolor="blue", color="white")),
            ft.Divider(height=10, color="transparent"),
            ft.Button("💼 Sou da Equipe / Dono", on_click=lambda e: mostrar_login_equipe(), height=60, style=ft.ButtonStyle(bgcolor="#333333", color="white"))
        ], horizontal_alignment=ft.CrossAxisAlignment.STRETCH)

        page.add(
            botao_tema,
            # ResponsiveRow: xs=12 (100% no celular), lg=4 (33% no computador)
            ft.ResponsiveRow([
                ft.Container(col={"xs": 12, "sm": 10, "md": 8, "lg": 4, "xl": 4}, content=coluna_central)
    ...
    def mostrar_login_equipe(mensagem=""):
        ...
        campo_login = ft.TextField(label="E-mail (Dono) ou Nome (Funcionário)")
        campo_senha = ft.TextField(label="Senha", password=True, can_reveal_password=True)
        msg_erro = ft.Text(value=mensagem, color="red", text_align=ft.TextAlign.CENTER)
        ...
        coluna_central = ft.Column([
            ft.Text("Acesso Restrito", size=30, weight="bold", color="purple", text_align=ft.TextAlign.CENTER),
            campo_login,
            campo_senha,
            ft.Button("Entrar", on_click=processar_login, height=50),
            ft.TextButton("Voltar ao Início", on_click=lambda e: mostrar_portal()),
            msg_erro
        ], horizontal_alignment=ft.CrossAxisAlignment.STRETCH)
    ...
    # ==========================================
    # 3. MÓDULO DO CLIENTE (Acesso e Boas-vindas)
    # ==========================================
    def mostrar_simulador_qr():
        ...
        campo_restaurante = ft.TextField(label="ID do Restaurante (Ex: 1)")
        campo_mesa = ft.TextField(label="Número da Mesa (Ex: 5)")
        msg_erro = ft.Text(value="", color="red", weight="bold", text_align=ft.TextAlign.CENTER)
        ...
        coluna_central = ft.Column([
            ft.Text("Simulador de QR Code", size=30, weight="bold", color="blue", text_align=ft.TextAlign.CENTER),
            ft.Text("Na vida real, a câmera leria isso automaticamente.", size=14, color="grey", text_align=ft.TextAlign.CENTER),
            campo_restaurante,
            campo_mesa,
            ft.Button("Acessar Mesa", on_click=acessar_mesa, height=50),
            ft.TextButton("Voltar", on_click=lambda e: mostrar_portal()),
            msg_erro
        ], horizontal_alignment=ft.CrossAxisAlignment.STRETCH)
    ...
    def mostrar_boas_vindas(id_mesa):
        ...
        campo_nome = ft.TextField(label="Qual o seu nome?")
        texto_resultado = ft.Text(value="", color="red", text_align=ft.TextAlign.CENTER)
        estado["carrinho"] = []
        ...
        coluna_central = ft.Column([
            ft.Text(f"Bem-vindo à Mesa {estado['numero_mesa']}!", size=30, weight="bold", text_align=ft.TextAlign.CENTER),
            campo_nome,
            ft.Button("Abrir Comanda", on_click=abrir_comanda_click, height=50),
            texto_resultado
        ], horizontal_alignment=ft.CrossAxisAlignment.STRETCH)
    ...
    def mostrar_cardapio_cliente():
        ...
        page.add(ft.Text(f"Comanda: {estado['nome_usuario']} (Mesa {estado['numero_mesa']})", size=20, weight="bold", color="blue", text_align=ft.TextAlign.CENTER))
        page.add(ft.Text("Cardápio", size=24, weight="bold"))

        aviso_pedido = ft.Text(value="", color="green", weight="bold", text_align=ft.TextAlign.CENTER)

        # Removido width=500, adicionado STRETCH para ocupar a tela toda
        coluna_cardapio = ft.Column(horizontal_alignment=ft.CrossAxisAlignment.STRETCH)
        ...
        def carregar_itens():
            ...
            for id_item, nome_item, preco_item, descricao_item, disponivel_bd in itens:
                ...
                # expand=True AQUI: Faz o texto empurrar o botão para a direita, mas quebra linha se faltar espaço
                info_produto = ft.Column([
                    ft.Text(f"{nome_item} - R$ {preco_item:.2f}", size=16, weight="bold"), 
                    ft.Text(descricao_item if descricao_item else "", size=14, color="grey"), 
                    status_visual
        ...
        page.add(ft.Row([ft.Button("Ver meus pedidos / Minha Conta", on_click=lambda e: mostrar_conta_cliente(), height=50, expand=True)]))
        page.update()
    ...
    def mostrar_conta_cliente():
        ...
        page.add(ft.Text(f"Meus Pedidos - {estado['nome_usuario']}", size=24, weight="bold", color="blue", text_align=ft.TextAlign.CENTER))

        area_carrinho = ft.Column(horizontal_alignment=ft.CrossAxisAlignment.STRETCH, spacing=10)
        coluna_pedidos_conta = ft.Column(horizontal_alignment=ft.CrossAxisAlignment.STRETCH)
        ...
        def atualizar_carrinho():
            ...
            area_carrinho.controls.append(ft.Text("🛒 Ordem de Pedido (Não enviados)", size=20, weight="bold", color="orange", text_align=ft.TextAlign.CENTER))

            if len(estado["carrinho"]) == 0:
                area_carrinho.controls.append(ft.Text("Sua ordem de pedido está vazia.", color="grey", text_align=ft.TextAlign.CENTER))
            else:
                ...
                for item in estado["carrinho"]:
                    def remover_item(e, item_rem=item):
                        ...
                        atualizar_carrinho()
                        page.update()
                    # expand=True no texto do item para não espremer o botão de remover
                    area_carrinho.controls.append(ft.Row([ft.Text(f"{item['nome']} - R$ {item['preco']:.2f}", weight="bold", expand=True), ft.Button("Remover", on_click=remover_item)], alignment=ft.MainAxisAlignment.SPACE_BETWEEN))

                area_carrinho.controls.append(ft.Text(f"Subtotal a enviar: R$ {subtotal:.2f}", color="orange", weight="bold", text_align=ft.TextAlign.CENTER))
                ...
                area_carrinho.controls.append(ft.Row([ft.Button("Confirmar e Enviar para Cozinha", on_click=confirmar_envio, height=50, expand=True)]))

        atualizar_carrinho()
        texto_total_valor = ft.Text("TOTAL CONFIRMADO: R$ 0.00", size=22, weight="bold", color="red", text_align=ft.TextAlign.CENTER)

        campo_pessoas = ft.TextField(label="Dividir por quantos?", expand=True)
        texto_divisao = ft.Text(value="", size=18, weight="bold", color="green", text_align=ft.TextAlign.CENTER)
        ...
        area_divisao = ft.Column([
            ft.Row([campo_pessoas, ft.Button("Calcular", on_click=calcular_divisao)]),
            texto_divisao
        ...
        def encerrar_comanda(e):
            ...
            page.vertical_alignment = ft.MainAxisAlignment.CENTER
            page.padding = 20
            page.add(ft.Text("Garçom chamado!", size=24, color="orange", weight="bold", text_align=ft.TextAlign.CENTER))
            page.add(ft.Text("Aguarde na mesa. O garçom está indo até você para realizar o pagamento.", size=18, text_align=ft.TextAlign.CENTER))
            page.add(ft.Row([ft.Button("Sair da Mesa", on_click=fazer_logout, height=50, expand=True)]))
            page.update()

        area_total.controls.extend([
            texto_total_valor,
            area_divisao,
            # wrap=True AQUI: Se a tela for pequena, os botões ficam um em cima do outro
            ft.Row([
                ft.Button("Voltar ao Cardápio", on_click=lambda e: mostrar_cardapio_cliente()), 
                ft.Button("Encerrar e Pagar", on_click=encerrar_comanda, style=ft.ButtonStyle(bgcolor="red", color="white"))
        ...
        def carregar_pedidos_conta():
            ...
            coluna_pedidos_conta.controls.append(ft.Text("🧾 Pedidos Confirmados (Na Cozinha)", size=20, weight="bold", color="green", text_align=ft.TextAlign.CENTER))

            total_conta = 0
            if len(meus_pedidos) == 0:
                coluna_pedidos_conta.controls.append(ft.Text("Você ainda não enviou nenhum pedido.", size=16, text_align=ft.TextAlign.CENTER))
            else:
